...
class nsquare_horses:
    ...
    def pintar_estado(self, estado):
        ...
        nrows, ncols = estado.shape
        width, height = 1.0 / ncols, 1.0 / nrows
        bkg_colors=['black', 'white']
        txt_colors=['white', 'black']

        # Add cells
        for (i,j), val in np.ndenumerate(estado):
            # Index either the first or second item of bkg_colors based on
            # a checker board pattern
            idx = [j % 2, (j + 1) % 2][i % 2]
            color = bkg_colors[idx]
            tb.add_cell(i, j, width, height, text=str(val) if val !=0 else "", 
                        loc='center', facecolor=color).set_text_props(color=txt_colors[idx])
